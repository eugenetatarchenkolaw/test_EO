...
| Приказ ГК «Роскосмос» № 116 от 21.05.2026 | Методика определения стоимости БРЕ. Зарегистрирован 26.06.2026 № 87243 | [Официальная публикация](http://publication.pravo.gov.ru/document/0001202606260063?ysclid=mu9zz6q3fn618390316&index=5) |
...
3. Каждая заказываемая зона — один валидный полигон без внутренних вырезов площадью не менее 1 км². Минимум проверяется до округления. Оплачиваемая площадь определяется по фактическому контуру, а не по ограничивающему прямоугольнику, затем приводится к 0,001 км². Минимум и геометрия — условия кейса, не универсальные требования ПП № 840 или рынка; [метод расчета площади](ORDER_GEOMETRY.md). Скидка округляется до 9 знаков. Цена за км² — до копеек. Стоимость позиции — до копеек. Используется `ROUND_HALF_UP`. Итог — сумма стоимостей позиций. Это **конвенция проверочного модуля**, а не заявленное требование ПП № 840. При другом правиле счета организатор обновляет модуль и контрольные примеры до старта.
