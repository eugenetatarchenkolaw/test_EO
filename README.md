...
Разработайте интеллектуальную систему, которая на основе спутниковых данных выявляет зоны затопления, оценивает потенциальный экономический ущерб для объектов инфраструктуры и определяет приоритетные территории для дополнительной космической съёмки.
Итоговая задача — сформировать экономически обоснованный план приобретения дополнительной спутниковой информации в пределах заданного бюджета и наглядно представить результаты для поддержки принятия решений при паводках.

В этом репозитории собраны: постановка, источники, форматы обмена и инструменты проверки, модель, расчет неопределенности, способ выбора съемки и продукт для пользователя разрабатывает команда.
...
Проверка контуров использует Rasterio и Shapely из `requirements.txt`; сам модуль арифметики цены работает на стандартной библиотеке. Проверить готовую сдачу:
...
Минимальная площадь одной зоны заказа — **1 км²**. Допускается произвольный валидный полигон без внутренних вырезов; оплачивается площадь по контуру. В `data/demo` — **10 объектов на одном условном чипе**, согласованные контуры и связи покрытия. [Геометрия заказа и проверка](docs/ORDER_GEOMETRY.md).
...
| Один каталог, зоны от 1 км² без holes, цена по контуру, соблюдение бюджета | Алгоритм поиска и ограничения на важные объекты |
...
Полный состав — в [контракте результатов](docs/CONTRACTS.md). Шкала: 20 критериев по 0–5, максимум 100. [Критерии](docs/EVALUATION.md) сгруппированы в технический блок (60 баллов) и отраслевой (40 баллов); номера и веса сохранены.
